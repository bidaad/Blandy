...
import { VwDepActiveInZone, columnStructure } from '../../../../model/viewModel/VwDepActiveInZone';
import { Modal } from 'react-bootstrap';
import DepActiveInZoneEdit from './DepActiveInZoneEdit';
...
type DepActiveInZoneProps =
    stateBase<VwDepActiveInZone> &
    typeof DepActiveInZoneStore.actionCreators &
    RouteComponentProps<{}> &
    { pId: string, selectRecord: any, editable: boolean, isSelected?: boolean, valuefiltersl: string, noselectId: string, typeForm: string };


class DepActiveInZone extends React.PureComponent<DepActiveInZoneProps, { showModal: boolean }> {
    ...
    getData = (pageNo: number, pageSize: number, filter: string, sort?: string, sortKey?: string) => {
        ...
        const rm: AdminModelRequest = { pageNo: pageNo, pageSize: pageSize, filter: filter, resourceName: "DepActiveInZone.Menu", isSelected: isSelected, sort: sort, sortKey: sortKey, parentId: this.props.pId, filtersl: this.props.valuefiltersl, noselectId: this.props.noselectId };
    ...
    editRecord = (id: string) => {
        ...
        //history.push('/DepActiveInZoneEdit', { id: id });
            ...
            <React.Fragment>
                ...
                <div key={2}>
                    <Modal size="lg" show={this.state.showModal} onHide={this.handleClose} className={Rtl}>
                        <Modal.Header >
                            <Modal.Title>
                                <TTitle {...{ name: "DepActiveInZone.EditForm" }} />
                            </Modal.Title>
                            <FontAwesomeIcon icon={faWindowClose} color="gray" size="lg" onClick={this.handleClose} />
                        </Modal.Header>
                        <Modal.Body>
                            <DepActiveInZoneEdit {...{ closeModal: this.handleClose, refreshGrid: this.refreshGrid, parentId: this.props.parentId }} />
                        </Modal.Body>
